...
| a character `Model` | `AuraTier` | number | The equipped `Gear.Aura` rung, written by `AuraService` (polled, like `ToolService`) and read by every client's `AuraController`, which draws the rings. Only the tier crosses the wire; the spinning rig is local to each viewer. **Absent while the owner has Settings' HIDE MY AURA on** (`data.hideAura`, `Net.HideAura`), so nobody draws it; the protection is unaffected. |
| a shop card's atom `Model` | *CollectionService tag* `AuraIcon` | — | Client-only (`AuraLook.icon`, in the shop card's ViewportFrame). `AuraController` animates the on-screen ones: it circles the card's camera and orbits the electrons (`AuraLook.poseIcon`). |
...
```lua
{
    ...
    locker  = {},   -- Quarantine Locker: finds parked at home, up to Tuning.LOCKER_SLOTS (8).
